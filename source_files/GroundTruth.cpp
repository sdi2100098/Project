...
int GroundTruth(const char *Query_path, const char *Ground_Truth_path, Graph *G, int k, int L, int *S)
{
    ...
    {
        // Reset the Distances array for each query
        for (int j = 0; j < G->number_of_indexes; j++)
            Distances[j] = -1.0f;

        sum = 0;
        Result = Filtered_Greedy_Search(G, i, k, L, S, &Q, Distances); // Greedy search for each vector
    ...
    // Calculate and print accuracy and results
    printf("\033[0;32m");
    ...
    // Calculate average time per query for both types
    ...
    // Total accuracy across all queries
