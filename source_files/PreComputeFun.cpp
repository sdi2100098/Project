...
int Graph_Graph_Dinstance_Precompute(Graph *G){
    ...
    const char *output_path = "Datasets/Small_Set/Graph_Graph_Precompute.txt";
    ...
    distances = (double *)malloc(sizeof(double) * G->number_of_indexes);
    if(distances == NULL){
    ...
    fprintf(output_file,"%d %d\n",G->number_of_indexes,G->number_of_indexes);
    if(fwrite(&G->number_of_indexes,sizeof(int),1,binary_file) != 1) goto fwrite_error; 
    if(fwrite(&G->number_of_indexes,sizeof(int),1,binary_file) != 1) goto fwrite_error; 
    ...
    for(int i = 0; i < G->number_of_indexes; i++){
        ...
        fprintf(output_file,"\n");
        if(fwrite(distances,sizeof(double),G->number_of_indexes,binary_file) != (size_t)G->number_of_indexes) goto fwrite_error;
    }
...
int Graph_Query_Dinstance_Precompute(Graph *G, Query *Q){
    ...
    const char *output_path = "Datasets/Small_Set/Graph_Query_Precompute.txt";
    ...
    double distance;
    double *distances = (double *)malloc(sizeof(double) * Q->number_of_indexes);
    if(distances == NULL){
        perror("Error in Graph_Graph_Dinstance_Precompute (fopen)");
        fclose(binary_file);
        fclose(output_file);
    ...
    fprintf(output_file,"%d %d\n",G->number_of_indexes,Q->number_of_indexes);
    if(fwrite(&G->number_of_indexes,sizeof(int),1,binary_file) != 1) goto fwrite_error;
    if(fwrite(&Q->number_of_indexes,sizeof(int),1,binary_file) != 1) goto fwrite_error;
    ...
    for(int i = 0; i < G->number_of_indexes; i++){
        ...
        fprintf(output_file,"\n");
        if(fwrite(distances,sizeof(double),Q->number_of_indexes,binary_file) != (size_t)Q->number_of_indexes) goto fwrite_error;
    }
